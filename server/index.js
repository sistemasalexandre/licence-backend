...
/* =====================================================================
   ...
   - cria usuário na tabela users (password_hash armazenado)
===================================================================== */
...
/* =====================================================================
   ...
   - valida senha e retorna hasLicense boolean
===================================================================== */
...
/* =====================================================================
   ...
   - associa license ao email em user_licenses e atualiza licenses
===================================================================== */
...
app.get('/api/has-license', async (req, res) => {
  try {
    ...
    const { data: ul, error } = await supabase.from('user_licenses').select('*').eq('user_email', email).limit(1);
    if (error) throw error;
...
/* =====================================================================
   ...
   POST /api/create-checkout-session { priceId, customerEmail? }
   - cria licença em status 'reserved' e contrói session com metadata
===================================================================== */
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    ...
    // gerar código temporário / license row reserved
    ...
    const sessionPayload = {
      ...
      success_url: (process.env.FRONTEND_URL || '') + '/auth.html?success=1',
      cancel_url: (process.env.FRONTEND_URL || '') + '/auth.html?cancel=1',
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    if (!stripe) { console.warn('Webhook recebido mas stripe não está configurado.'); return res.status(200).send({ received: true }); }
    ...
    try {
      if (STRIPE_WEBHOOK_SECRET) {
        ...
        // se não tem webhook secret, tenta parse simples (menos seguro)
          ...
          if (license) {
            ...
            if (customer_email) {
              ...
              if (user) {
                ...
                // Keep used_by as email; when user registers later they can be matched (or webhook could create placeholder user)
                // Optionally create placeholder user here — we avoid by default.
              }
            }
          }
        } catch (procErr) {
...
/* =====================================================================
   Debug: listar licenses (opcional)
===================================================================== */
